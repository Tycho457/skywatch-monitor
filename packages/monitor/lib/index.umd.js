;(function (global, factory) {
  ...
  var ReportInfo = /** @class */ (function () {
    ...
    ReportInfo.prototype.send = function (data, isTmmediate) {
      ...
      isTmmediate ? fn() : this.queue.push(fn)
    }
  ...
  // 观察者
  function observe(
    type, // 观察的目的类型，例如；'paint' 'resource' 'navigation'等
    handler,
  ) {
  ...
  function getLCP(setStore) {
    if (!isPerformanceObserver()) {
      ...
      var entryHandler = function (entry) {
        ...
        lcpDone = true
        observer_1 && observer_1.disconnect()
        setStore(PerformanceType.LCP, {
          value: entry.startTime.toFixed(2),
          event: entry.name,
        })
      }
      var observer_1 = observe(PerformanceType.LCP, entryHandler)
    }
  }
  ...
  function getFID(setStore) {
    if (!isPerformanceObserver()) {
      ...
      var observer_1 = observe(PerformanceType.FID, entryHandler)
    }
  }
  ...
  function getFSP(setStore) {
    if (!MutationObserver) {
      throw new Error('当前浏览器不支持 MutationObserver')
    }
  ...
  function getWhiteScreen(setStore, options) {
    ...
    var startSampLists = []
    var nowSampLists = []
    var containerLists = ['html', 'body', '#app', '#root'] // 不是渲染的父容器
    var timer = null
    ...
    function onSamp() {
      ...
      for (var i = 0; i <= 9; i++) {
        ...
        isContainer(xElements[0]) && points++
        // 避免重复计算
        i != 5 && isContainer(yElements[0]) && points++
      }
  ...
  var Error$1 = /** @class */ (function () {
    ...
    // vue 错误
    Error.prototype.handleVueError = function (vue) {
      var _this = this
    ...
    // ajax 错误
    Error.prototype.handleAjaxError = function () {
      var _this = this
  ...
  var Monitor = /** @class */ (function () {
    ...
    Monitor.prototype.init = function (options) {
      ...
      options.isCollectErr && new Error$1(options)
    }
    // 校验相关
